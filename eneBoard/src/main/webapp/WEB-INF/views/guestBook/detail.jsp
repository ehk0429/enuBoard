	...
	<div class="container">
		<div class="row">
			<div class="col-lg-12">
				<div class="card">
					<div class="card-body">
						<form role="form" action="/guestBook/remove?id=${item.id}" method="post"
							id="form">
							<h5 class="card-title">제목: ${item.title}</h5>
							<h6 class="card-subtitle mb-2 text-muted">writer
								:${item.writer } created-date: ${item.created_at }</h6>
							<p class="card-text">${item.content }</p>
							<p class="card-text">${item.is_enabled ? 'secret' : 'public'}</p>
						</form>
						<a class="btn btn-sm btn-outline-secondary" href="list"
							role="button">목록</a> <a class="btn btn-sm btn-outline-info"
							href='modify?id=${item.id}'>수정</a>
